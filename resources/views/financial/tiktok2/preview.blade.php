...
@section('title', 'Preview Data Keuangan TikTok')
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">Preview Data Keuangan TikTok</h3>
                    <div class="card-tools">
                        <a href="{{ route('finance.tiktok2.import') }}" class="btn btn-default btn-sm">
                ...
                <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped table-sm">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th>Seller Shipping Fee</th>
                                    <th>Voucher Xtra Service Fee</th>
                                    <th>Cashback Fee</th>
                                    <th>Biaya 6</th>
                                    <th>Biaya 7</th>
                                    <th>Biaya 8</th>
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $index % 2 == 0 ? 'table-primary' : '' }}">
                                    ...
                                    <td class="text-end">{{ isset($row['nominal_diskon7']) ? number_format($row['nominal_diskon7'], 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['nominal_diskon8']) ? number_format($row['nominal_diskon8'], 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['nominal_diskon9']) ? number_format($row['nominal_diskon9'], 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['nominal_diskon10']) ? number_format($row['nominal_diskon10'], 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['nominal_diskon11']) ? number_format($row['nominal_diskon11'], 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['nominal_diskon12']) ? number_format($row['nominal_diskon12'], 0, ',', '.') : '-' }}</td>
                    ...
                    <div class="mt-3">
                        <form action="{{ route('finance.tiktok2.import-process') }}" method="POST" class="d-inline">
                            @csrf
                            <!-- Use session token instead of sending all data via POST -->
                            <input type="hidden" name="process_token" value="{{ session('tiktok2_process_token', uniqid()) }}">
                            
                            <button type="submit" class="btn btn-success" {{ $validRows == 0 ? 'disabled' : '' }}>
                                <i class="fas fa-check"></i> Proses {{ $validRows }} Data Valid
                            </button>
                            
                            <a href="{{ route('finance.tiktok2.import') }}" class="btn btn-secondary">
                                <i class="fas fa-arrow-left"></i> Kembali
                            </a>
                        </form>
                    </div>
