            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    <h5 class="m-0 fw-bold text-primary">
                        <i class="fas fa-table me-2"></i>{{ __('Preview Data Import') }} - Blibli
                    </h5>
                </div>
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-12">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb bg-light py-2 px-3 rounded">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{ route('sales.platform', ['platform' => 'blibli']) }}" class="text-decoration-none">Platform Blibli</a></li>
                                    <li class="breadcrumb-item"><a href="{{ route('sales.blibli.import-excel') }}" class="text-decoration-none">Import Excel Blibli</a></li>
                                    ...
                                    <div class="d-flex flex-column">
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            <div>
                                                ...
                                                <h5 class="mb-0 fw-bold">{{ isset($skippedRows) ? collect($skippedRows)->pluck('count')->sum() : 0 }}</h5>
                                            </div>
                                        </div>
                                        <div class="d-flex align-items-center">
                                    ...
                                    <div class="d-flex flex-column">
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            <div>
                                                ...
                                                <h5 class="mb-0 fw-bold">{{ isset($unmappedProducts) ? count($unmappedProducts) : 0 }}</h5>
                                            </div>
                                        </div>
                                        <div class="d-flex align-items-center">
                                            <div class="bg-danger text-white rounded-circle p-2 me-3" style="width: 36px; height: 36px; display: flex; align-items: center; justify-content: center;">
                                            ...
                                            <div>
                                                ...
                                                <h5 class="mb-0 fw-bold">{{ isset($invalidData) ? count($invalidData) : 0 }}</h5>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @if(isset($duplicateOrders) && count($duplicateOrders) > 0)
                        <div class="alert alert-warning alert-dismissible fade show" role="alert">
                            <h5 class="alert-heading d-flex align-items-center">
                                <i class="fas fa-copy me-2"></i> Order Duplikat dalam Database:
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            ...
                            <div class="row">
                                @foreach($duplicateOrders as $orderNumber)
                                    <div class="col-md-4 col-sm-6">
                                        <span class="badge bg-danger text-white border mb-2 py-2 px-3 w-100 text-start">
                                            <i class="fas fa-tag me-2 text-white"></i>{{ $orderNumber }}
                                        </span>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    {{-- Alert jika semua order sudah ada di database --}}
                    @if(isset($allAlreadyInDb) && $allAlreadyInDb)
                        <div class="alert alert-warning alert-dismissible fade show" role="alert">
                            <h5 class="alert-heading d-flex align-items-center">
                                <i class="fas fa-exclamation-triangle me-2"></i> Perhatian!
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            <p>Semua order dalam file ini sudah ada di database. Tidak ada data yang dapat diimport.</p>
                        </div>
                    @endif
                    ...
                    @if(isset($duplicateOrdersInFile) && count($duplicateOrdersInFile) > 0)
                        @php
                            // Hitung jumlah kemunculan setiap order number di $data
                            $orderCounts = [];
                            foreach ($data as $item) {
                                $orderCounts[$item['no_order']] = ($orderCounts[$item['no_order']] ?? 0) + 1;
                            }
                            // Filter hanya order yang muncul lebih dari 1 kali
                            $realDuplicates = array_filter($orderCounts, function($count) { return $count > 1; });
                        @endphp
                        @if(count($realDuplicates) > 0)
                            <div class="alert alert-info alert-dismissible fade show" role="alert">
                                <h5 class="alert-heading d-flex align-items-center">
                                    <i class="fas fa-info-circle me-2"></i> Order Duplikat dalam File:
                                </h5>
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                <p>File berisi nomor order duplikat. Untuk Blibli, nomor order yang sama diperbolehkan dalam satu file. Hanya kemunculan pertama yang akan diproses:</p>
                                <div class="row">
                                    @foreach(array_keys($realDuplicates) as $orderNumber)
                                        <div class="col-md-4 col-sm-6">
                                            <span class="badge bg-info text-white mb-2 py-2 px-3 w-100 text-start">
                                                <i class="fas fa-copy me-2"></i>{{ $orderNumber }}
                                            </span>
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    {{-- Alert informasi order yang akan diimport --}}
                    @if(isset($data) && count($data) > 0 && (!isset($allAlreadyInDb) || !$allAlreadyInDb))
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            <h5 class="alert-heading d-flex align-items-center">
                                <i class="fas fa-check-circle me-2"></i> Order Siap untuk Import:
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            <p>Order berikut akan diimport ke database:</p>
                            <div class="row">
                                @php
                                    $uniqueOrders = array_unique(array_column($data, 'no_order'));
                                    $newOrders = array_diff($uniqueOrders, $duplicateOrders ?? []);
                                @endphp
                                @foreach(array_slice($newOrders, 0, 10) as $orderNumber)
                                    <div class="col-md-4 col-sm-6">
                                        <span class="badge bg-success text-white mb-2 py-2 px-3 w-100 text-start">
                                            <i class="fas fa-plus me-2"></i>{{ $orderNumber }}
                                        </span>
                                    </div>
                                @endforeach
                                @if(count($newOrders) > 10)
                                    <div class="col-12">
                                        <small class="text-muted">... dan {{ count($newOrders) - 10 }} order lainnya</small>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    @if(isset($data) && count($data) > 0)
                        @php
                            $allAlreadyInDb = true;
                            foreach($data as $item) {
                                if (empty($item['already_in_db'])) {
                                    $allAlreadyInDb = false;
                                    break;
                                }
                            }
                        @endphp
                        @if($allAlreadyInDb)
                            <div class="alert alert-warning mb-3">
                                <strong>Semua nomor order pada file ini sudah ada di database. Tidak ada data yang bisa diimport.</strong>
                            </div>
                        @endif
                    @endif
                                        ...
                                        @if(!empty($data))
                                            ...
                                            @foreach($orderItems as $orderNumber => $items)
                                                ...
                                                @foreach($items as $idx => $item)
                                                    <tr class="{{ $rowClass }} {{ $idx === 0 ? 'new-order' : '' }}">
                                                        @if($idx == 0)
                                                            <td class="text-center" rowspan="{{ $rowspan }}">{{ $no++ }}</td>
                                                            <td class="text-center" rowspan="{{ $rowspan }}">{{ isset($item['tanggal']) ? (is_object($item['tanggal']) ? $item['tanggal']->format('Y-m-d') : $item['tanggal']) : '-' }}</td>
                                                            <td class="text-center" rowspan="{{ $rowspan }}">{{ $item['hari'] ?? '-' }}</td>
                                                            <td class="text-center" rowspan="{{ $rowspan }}">
                                                                <span class="fw-bold text-primary">{{ $item['no_order'] }}</span>
                                                                @if(in_array($item['no_order'], $duplicateOrders ?? []))
                                                                    <span class="badge bg-danger ms-1">Sudah Ada di Database</span>
                                                                @endif
                                                            </td>
                                                        @endif
                                                        ...
                                                        <td class="text-end">{{ number_format($item['harga_setelah_diskon'], 0, ',', '.') }}</td>
                                                        <td class="text-end">{{ number_format($item['qty'] * $item['harga_setelah_diskon'], 0, ',', '.') }}</td>
                                                        <td class="text-center">{{ $item['no_resi'] ?? '-' }}</td>
                                                        <td class="text-center">{{ $item['status_hari'] ?? '-' }}</td>
                                                        @if($idx == 0)
                                                            <td class="text-end fw-bold" rowspan="{{ $rowspan }}">{{ number_format($orderTotals[$orderNumber], 0, ',', '.') }}</td>
                                                        @endif
                                                    </tr>
                                                @endforeach
                                            @endforeach
                    ...
                    <!-- Form untuk proses import -->
                    <form action="{{ route('sales.blibli.process-import') }}" method="POST" class="mt-4">
                        @csrf
                        @php
                            // Hitung apakah tombol harus dinonaktifkan
                            $uniqueOrdersInData = isset($data) ? array_unique(array_column($data, 'no_order')) : [];
                            $duplicateOrdersCount = isset($duplicateOrders) ? count($duplicateOrders) : 0;
                            $uniqueOrdersCount = count($uniqueOrdersInData);
                            $allOrdersInDb = $uniqueOrdersCount > 0 && $duplicateOrdersCount >= $uniqueOrdersCount;
                            $shouldDisableButton = !$canProceed || $allOrdersInDb || (isset($allAlreadyInDb) && $allAlreadyInDb);
                        @endphp
                        <div class="row">
                            <div class="col-md-6">
                                <a href="{{ route('sales.blibli.import-excel') }}" class="btn btn-outline-secondary">
                                    <i class="fas fa-arrow-left me-1"></i> Kembali
                                </a>
                            </div>
                            <div class="col-md-6 text-end">
                                <button type="submit" class="btn btn-primary" 
                                    @if($shouldDisableButton) disabled @endif>
                                    <i class="fas fa-file-import me-1"></i> Import Data
                                </button>
                                
                                @if(!$canProceed)
                                    <div class="text-danger small mt-2">
                                        @if(isset($unmappedProducts) && count($unmappedProducts) > 0)
                                            <i class="fas fa-exclamation-circle me-1"></i> Selesaikan mapping produk terlebih dahulu
                                        @else
                                            <i class="fas fa-exclamation-circle me-1"></i> Tidak dapat melanjutkan import
                                        @endif
                                    </div>
                                @endif
                                ...
                                @if($allOrdersInDb)
                                    <div class="text-warning small mt-2">
                                        <i class="fas fa-exclamation-circle me-1"></i> Semua order sudah ada di database
                                </div>
                                @endif
                            </div>
                        </div>
