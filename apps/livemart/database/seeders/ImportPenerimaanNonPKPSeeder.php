...
class ImportPenerimaanNonPKPSeeder extends Seeder
{
    /**
    ...
    /**
     * Store the command from ImportPenerimaanNonPKP
     */
    ...
    {
        if ($this->command) {
            $this->command->info('Importing Penerimaan Non-PKP (Tax: LM)...');
        }
        
        // Check if the CSV file exists
        $csvPath = storage_path('app/imports/barangdatang/MASTER - NON PKP.csv');
        ...
        // Get Tax Category LM
        $taxCategory = TaxCategory::where('name', 'LM')->first();
        if (!$taxCategory) {
            if ($this->command) {
                $this->command->error('Tax Category LM not found!');
            }
            return;
        }
        ...
        try {
            $date = Carbon::now();
            $poNumber = 'PO-IMPORT-NONPKP-' . $date->format('YmdHis');
            $kodePenerimaan = 'GR-NONPKP-' . $date->format('YmdHis');
            ...
            $penerimaan = Penerimaan::create([
                ...
                'status' => 'Unlocated',
                'catatan' => 'Imported from MASTER - NON PKP.csv with Tax LM',
