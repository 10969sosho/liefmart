...
class SalesVolumeReportHandler implements ExportHandlerInterface
{
    ...
    public function handle(array $filters): array
    {
        ...
        // Execute query using SalesVolumeQuery
        $query = SalesVolumeQuery::build($queryFilters);
        $results = DB::select($query);

        // Get summary
        $summaryQuery = SalesVolumeQuery::buildSummary($queryFilters);
        ...
        $filename = 'laporan-volume-penjualan-' . date('Y-m-d') . '.xlsx';

        $headings = ['No', 'Order Number', 'Tanggal', 'Platform', 'Total Volume (pcs)', 'Total Value (Rp)'];
