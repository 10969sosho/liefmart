...
class MultipleItemReportHandler implements ExportHandlerInterface
{
    public function getType(): string
    {
        return 'multiple_item_report';
    }
    ...
    public function handle(array $filters): array
    {
        ...
        $page = 1;
        $perPage = 1000000;
        $query = MultipleItemQuery::build($queryFilters, $perPage, $page);
        $results = DB::select($query);

        // Get summary
        $summaryQuery = MultipleItemQuery::buildSummary($queryFilters);
        ...
        $orders = collect($results)->map(function ($row) {
            return [
                ...
                'Product Names' => $row->product_names ?? '-',
                'Item Count' => (int)($row->item_count ?? 0),
        ...
        $headings = [
            'No', 'Order Number', 'Tanggal', 'Platform', 'Product Names',
            'Item Count', 'Total Value', 'Total Volume',
        ];

        $filename = 'laporan-multiple-item-' . date('Y-m-d') . '.xlsx';
