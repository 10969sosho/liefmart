...
<div class="container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="mb-4">
                <h6 class="mb-2 fw-bold"><i class="fa-regular fa-calendar me-2"></i>Filter Cepat:</h6>
                <div class="btn-group" role="group">
                    <a href="{{ route('analytics.sales-by-master-product', ['quick_range' => '7days'] + request()->except(['start_date', 'end_date', 'quick_range'])) }}"
                       class="btn {{ request('quick_range') == '7days' ? 'btn-primary' : 'btn-outline-primary' }}">
                        <i class="fa-regular fa-calendar me-1"></i> 7 Hari
                    </a>
                    <a href="{{ route('analytics.sales-by-master-product', ['quick_range' => '2weeks'] + request()->except(['start_date', 'end_date', 'quick_range'])) }}"
                       class="btn {{ request('quick_range') == '2weeks' ? 'btn-primary' : 'btn-outline-primary' }}">
                        <i class="fa-regular fa-calendar me-1"></i> 2 Minggu
                    </a>
                    <a href="{{ route('analytics.sales-by-master-product', ['quick_range' => '1month'] + request()->except(['start_date', 'end_date', 'quick_range'])) }}"
                       class="btn {{ request('quick_range') == '1month' ? 'btn-primary' : 'btn-outline-primary' }}">
                        <i class="fa-regular fa-calendar me-1"></i> 1 Bulan
                    </a>
                    <a href="{{ route('analytics.sales-by-master-product', ['quick_range' => '3months'] + request()->except(['start_date', 'end_date', 'quick_range'])) }}"
                       class="btn {{ request('quick_range') == '3months' ? 'btn-primary' : 'btn-outline-primary' }}">
                        <i class="fa-regular fa-calendar me-1"></i> 3 Bulan
                    </a>
            ...
            <form method="GET" action="{{ route('analytics.sales-by-master-product') }}" id="filter-form" class="card mb-4">
                <div class="card-header bg-light" style="background-color: #f8f9fa !important; color: #333;">
                    <h6 class="mb-0 fw-bold"><i class="fa-solid fa-sliders me-2"></i>Filter Custom</h6>
                </div>
                <div class="card-body">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6">
                            <button type="button" class="btn btn-success w-100" onclick="exportMasterProduct()">
            ...
            <div class="bg-light p-3 rounded mb-4 border">
                <h6 class="fw-bold mb-3"><i class="fa-solid fa-filter me-2"></i>Filter Aktif:</h6>
                <div class="row">
                    <div class="col-md-3">
...
@include('analytics.partials.export_script', ['exportType' => 'sales_by_master_product'])
@endsection
...
<script>
    (function() {
        ...
        function reloadDropdown(url, payload, targetId, callback = null) {
            ...
            fetch(fullUrl)
                ...
                .then(data => {
                    ...
                    // Prepare options data for TomSelect
                    ...
                    // Add options to select element FIRST before initializing TomSelect
                    if (validData.length > 0) {
                        validData.forEach(item => {
                            const option = document.createElement('option');
                            option.value = String(item.id);
                    ...
                    // Add initial data to TomSelect IMMEDIATELY
                    if (initialData.length > 0) {
                    ...
                    // Restore selected values AFTER options are added
        ...
        function initTomSelect(selectId, url = null, initialData = null) {
            ...
            // Add initial data as native options FIRST
            ...
            // Add initial data to TomSelect IMMEDIATELY if available
            if (initialData && initialData.length > 0) {
        ...
        function setupSubBrandCascade(tsInstance) {
            ...
            const changeHandler = debounce(function() {
                // Skip cascade during initial load
                if (isInitialLoad) {
            ...
            // Also listen to TomSelect add/remove events
        ...
        function initCascadingSelects() {
            // Set flag to prevent cascade during initial load
            isInitialLoad = true;
            ...
            // After all initializations complete, allow cascade
            setTimeout(() => {
                isInitialLoad = false;
        ...
        window.exportMasterProduct = function() {
            ...
            dispatchExport('sales_by_master_product', filters);
        };
    })();
</script>
