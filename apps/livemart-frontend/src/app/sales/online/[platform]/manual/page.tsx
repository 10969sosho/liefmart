...
const onlineManualSchema = z.object({
  order_number: z.string().min(1, 'No. order wajib diisi'),
...
type OnlineManualFormData = z.infer<typeof onlineManualSchema>;
...
export default function SalesOnlineManualPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<OnlineManualFormData>({
    resolver: zodResolver(onlineManualSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: OnlineManualFormData) => {
    if (items.length === 0) {
    ...
    try {
      ...
      const response = await salesApi.storeOnlineManual(platform, {
        ...data,
      ...
      if (response.success) {
        ...
        setError(response.message || 'Gagal menyimpan order');
      }
    } catch (err: any) {
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-bold tracking-tight">
                    Input Manual - {platformNames[platform] || platform}
                  </h1>
                  <p className="text-muted-foreground">
                    Input order penjualan online secara manual
                  </p>
                </div>
                <Button
                  variant="outline"
                  ...
                  <CardContent>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        <Label htmlFor="order_number">No. Order *</Label>
                        <Input
                          id="order_number"
                          type="text"
                          placeholder="Contoh: 240804001"
                          {...register('order_number')}
                          disabled={isLoading}
                        />
                        {errors.order_number && (
                          <p className="text-sm text-red-600">{errors.order_number.message}</p>
                        )}
                      </div>
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead>Nama Produk Platform</TableHead>
                          ...
                          {items.map((item, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell>
                                <Input
                                  type="text"
                                  value={item.platform_product_name}
                                  onChange={(e) => updateItem(index, 'platform_product_name', e.target.value)}
                                  placeholder="Nama produk di platform"
                                  disabled={isLoading}
                                />
                              </TableCell>
                              <TableCell>
