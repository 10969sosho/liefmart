...
export default function FinanceImportPage() {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const response = await financeApi.previewImport(platform, file);
  ...
  const handleProcessImport = async () => {
    ...
    if (!confirm('Apakah Anda yakin ingin mengimport data pembayaran ini?')) {
      return;
    }
    ...
    try {
      ...
      const response = await financeApi.processImport(platform, previewData);

      if (response.success) {
        setSuccess('Import berhasil! Invoice akan digenerate otomatis.');
        setTimeout(() => {
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-bold tracking-tight">
                    Import Pembayaran - {platformNames[platform] || platform}
                  </h1>
                  <p className="text-muted-foreground">
                    Upload file Excel untuk import data pembayaran
                  </p>
                </div>
                <Button
                  variant="outline"
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                          ...
                          <tbody>
                            {previewData.rows?.slice(0, 50).map((row: any, index: number) => (
                              <tr key={index} className={`border-b ${row.has_error ? 'bg-red-50' : ''}`}>
                                ...
                                <td className="px-4 py-2 text-sm">{row.payment_date || '-'}</td>
                                <td className="px-4 py-2 text-sm">Rp {row.amount?.toLocaleString('id-ID') || 0}</td>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isProcessing ? 'Processing...' : `Import ${previewData.valid_rows} Pembayaran`}
                      </Button>
                      <Button
                        variant="outline"
                        onClick={() => {
              ...
              <Card>
                ...
                <CardContent>
                  <ol className="list-decimal list-inside space-y-2 text-sm text-muted-foreground">
                    ...
                    <li>Pastikan kolom-kolom berikut ada: Order Number, Payment Date, Amount</li>
                    <li>Jangan ubah format template</li>
                    ...
                    <li>Sistem akan otomatis:
                      <ul className="list-disc list-inside ml-6 mt-1 space-y-1">
                        <li>Matching dengan order yang ada</li>
                        <li>Generate nomor invoice</li>
                        <li>Update status pembayaran order</li>
                      </ul>
                    </li>
                  </ol>
                </CardContent>
