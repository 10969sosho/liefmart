...
public function shopeeFinancialTransactions()
    ...
    public function isFullyReturned()
    {
        ...
        $hasRetur = \App\Models\ReturPenjualan::where('order_id', $this->id)
        ...
        $remainingQty = (float) $this->orderItems->sum('quantity');
        if ($remainingQty <= 0.0001) {
            return true;
        }

        $totalShippedUnits = (float) \App\Models\BarangKeluar::whereHas('orderItem', function ($q) {
        ...
        $totalReturnedUnits = (float) \App\Models\ReturPenjualanDetail::whereHas('returPenjualan', function($q) {
                $q->where('order_id', $this->id)
                  ->where('status', 'selesai');
            })
            ->sum('qty');
