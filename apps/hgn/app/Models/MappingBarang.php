...
class MappingBarang extends Model
{
    ...
    public static function getMappingsForOrderCreatedAt($platformProductId, $orderCreatedAt)
    {
        ...
        $effectiveVersionRow = static::query()
            ->where('platform_product_id', $platformProductId)
            ->whereRaw('COALESCE(valid_from, created_at) <= ?', [$orderCreatedAt])
            ->selectRaw('version, MAX(COALESCE(valid_from, created_at)) as effective_at')
            ->groupBy('version')
            ->orderByDesc('effective_at')
            ->first();

        if ($effectiveVersionRow && $effectiveVersionRow->version !== null) {
            return static::where('platform_product_id', $platformProductId)
                ->where('version', $effectiveVersionRow->version)
                ->get();
        }
