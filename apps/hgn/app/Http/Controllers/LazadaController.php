            ...
            if (!empty($data)) {
                ...
                foreach ($data as $row) {
                    ...
                    if (!$platformProductId) {
                        ...
                        $variation = isset($row['variasi']) && trim($row['variasi']) !== '' ? trim($row['variasi']) : '';
        ...
        try {
            ...
            if ($existingOrder) {
                ...
                return redirect()->route('sales.lazada.manual')
                    ->with('error', 'Nomor pesanan sudah ada di database.')
                    ->withInput();
            }

            // Cari platform product berdasarkan produk dan varian
            $platformProduct = PlatformProduct::where('platform_id', $this->platform->id)
                ->where('platform_product_name', $request->produk)
                ->where('variant', $request->varian ?? '')
                ->first();
            ...
            if (!$platformProduct) {
                DB::rollBack();
                return redirect()->route('sales.lazada.manual')
                    ->with('error', "Produk '{$request->produk}' tidak ditemukan di database. Pastikan produk sudah terdaftar sebagai Platform Product.")
                    ->withInput();
            }

            // Verifikasi stok terlebih dahulu
            $checkStockResult = $this->checkStock($platformProduct, $request->qty);
            if (!$checkStockResult['success']) {
                DB::rollBack();
                return redirect()->route('sales.lazada.manual')
                    ->with('error', "Stok tidak cukup untuk produk {$checkStockResult['product_name']}")
                    ->withInput();
            }
            ...
            $order = Order::create([
                ...
                'tanggal' => $tanggal,
                'hari' => $request->hari,
                'status_hari' => $request->status_hari,
                'status' => 'imported',
            ]);
            ...
            $orderItem = OrderItem::create([
                ...
                'price_after_discount' => $request->harga_setelah_diskon,
                'tracking_number' => null,
            ]);

            // Kurangi stok sesuai mapping dan catat barang keluar
            $this->reduceStock($platformProduct, $request->qty, $orderItem);

            DB::commit();

            return redirect()->route('sales.lazada.index')
                ->with('success', 'Order berhasil ditambahkan dan barang keluar telah dicatat!');

        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Error in storeManual: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
