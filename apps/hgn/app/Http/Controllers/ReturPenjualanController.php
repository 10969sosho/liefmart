...
class ReturPenjualanController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($oldOrderId) {
            $selectedOrder = Order::query()
                ->select(['id', 'platform_id', 'order_number', 'tanggal'])
                ->with(['platform:id,name'])
                ->find($oldOrderId);
        }

        return view('retur.penjualan.create', compact('selectedOrder'));
    }
    ...
    public function searchOrder(Request $request)
    {
        ...
        if ($mainCategoryId) {
            $query
                ...
                ->join('warehouse_stock', 'warehouse_stock.id', '=', 'order_items.warehouse_stock_id')
                ->join('products', 'products.id', '=', 'warehouse_stock.product_id')
                ->where('products.main_category_id', $mainCategoryId)
                ->groupBy('orders.id', 'orders.platform_id', 'orders.order_number', 'orders.tanggal');
        } else {
        ...
        $results = $orders->map(function ($order) {
            ...
            if (!empty($order->tanggal)) {
                try {
                    $tanggal = Carbon::parse($order->tanggal)->format('d/m/Y');
                } catch (\Throwable $e) {
                    $tanggal = '-';
                }
            }
            ...
            return [
                'id' => $order->id,
                'text' => $order->order_number . ' - ' . $platformName . ' - ' . $tanggal,
            ];
        })->values();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $detailItems = collect($validated['details'] ?? [])
                ->filter(function ($detail) {
                    return isset($detail['qty']) && floatval($detail['qty']) > 0;
                })
                ->values();

            if ($detailItems->isEmpty()) {
                throw ValidationException::withMessages([
                    'details' => 'Minimal 1 barang harus memiliki qty retur > 0.',
                ]);
            }
        ...
        try {
            ...
            foreach ($detailItems as $index => $detail) {
                ...
                $createdAnyDetailForThisOrderItem = false;
                foreach ($mappings as $mapping) {
                    ...
                    if (!isset($barangKeluarByProduct[$productId]) || empty($barangKeluarByProduct[$productId])) {
                        ...
                        throw new \Exception("Barang keluar tidak ditemukan untuk product ID {$productId} pada order item ID {$orderItem->id}. Periksa mapping barang dan data barang keluar.");
                    }
                    ...
                    if ($qtyForThisProduct > 0) {
                        ...
                        if ($existingDetail) {
                            ...
                            $existingDetail->save();
                            $createdAnyDetailForThisOrderItem = true;
                ...
                if (!$createdAnyDetailForThisOrderItem) {
                    throw new \Exception("Retur gagal diproses untuk order item ID {$orderItem->id} karena tidak ada detail retur yang berhasil dibuat. Periksa mapping barang dan data barang keluar.");
                }
