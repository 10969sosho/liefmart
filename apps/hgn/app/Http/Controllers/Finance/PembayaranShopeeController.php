...
class PembayaranShopeeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $profileId = $profile ? (string) Str::uuid() : null;
        $profileStart = $profile ? microtime(true) : null;
        $profileMarks = [];
        $profileSlowQueries = [];

        $mark = function (string $label) use ($profile, $profileStart, &$profileMarks): void {
            if (!$profile || $profileStart === null) {
                return;
            }

            $profileMarks[$label] = round((microtime(true) - $profileStart) * 1000, 2);
        };

        if ($profile) {
            Log::info('finance.shopee.index.profile.start', [
                'id' => $profileId,
                'url' => $request->fullUrl(),
                'filters' => $request->except(['_token']),
            ]);

            DB::listen(function ($query) use (&$profileSlowQueries, $profileId) {
                $timeMs = (float) ($query->time ?? 0);
                if ($timeMs < 200) {
                    return;
                }

                if (count($profileSlowQueries) >= 25) {
                    return;
                }
            ...
            register_shutdown_function(function () use (&$profileMarks, &$profileSlowQueries, $profileId, $profileStart) {
                $error = error_get_last();
                $elapsedMs = $profileStart ? round((microtime(true) - $profileStart) * 1000, 2) : null;

                Log::warning('finance.shopee.index.profile.shutdown', [
                    'id' => $profileId,
                    'elapsed_ms' => $elapsedMs,
                    'marks_ms' => $profileMarks,
                    'slow_queries' => $profileSlowQueries,
                    'slow_queries_count' => count($profileSlowQueries),
                    'last_error' => $error,
                ]);
            });
        }

        // Get platform by ID (shopee = ID 1)
        $platformModel = Platform::find(1);
        
        // Jika tidak ditemukan berdasarkan ID, cari berdasarkan nama dengan case-insensitive
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) = ?', ['shopee'])->first();
        }
        
        // Jika masih tidak ditemukan, cari dengan LIKE
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) LIKE ?', ['%shopee%'])
                ->whereRaw('LOWER(name) NOT LIKE ?', ['%shopee2%'])
                ->first();
        }
        
        if (!$platformModel) {
            return redirect()->back()->with('error', 'Platform shopee tidak ditemukan');
        }
        $platformId = $platformModel->id;
        $platform = 'shopee'; // Set platform name for view
        $mark('platform_resolved');
        ...
        $totals = cache()->remember($cacheKey, 300, function() use ($request) { // Cache for 5 minutes
            $totalsQuery = ShopeeFinancialTransaction::query();
        ...
        $mark('transactions_paginated');
        
        $missingOrdersBase = Order::withoutGlobalScope('mainCategory')
            ->leftJoin('shopee_financial_transactions as sft', 'orders.id', '=', 'sft.order_id')
            ->join('order_items as oi', 'orders.id', '=', 'oi.order_id')
            ->join('warehouse_stock as ws', 'ws.id', '=', 'oi.warehouse_stock_id')
            ->join('products as p', 'p.id', '=', 'ws.product_id')
            ->whereNull('sft.order_id')
            ->where('orders.platform_id', $platformId)
            ->where('oi.quantity', '>', 0);

        if ($mainCategoryId) {
            $missingOrdersBase->where('p.main_category_id', $mainCategoryId);
        }

        $missingOrdersCount = (clone $missingOrdersBase)
            ->distinct()
            ->count('orders.id');
        $mark('missing_orders_count');

        $missingOrdersNominal = cache()->remember(
            'shopee_missing_nominal_' . (string) ($mainCategoryId ?? 'all'),
            300,
            function () use ($missingOrdersBase) {
                return (clone $missingOrdersBase)
                    ->selectRaw('COALESCE(SUM(oi.price_after_discount * oi.quantity), 0) as total')
                    ->value('total') ?? 0;
            }
        );
        $mark('missing_orders_nominal');

        $missingOrders = (clone $missingOrdersBase)
            ->select(['orders.id', 'orders.order_number', 'orders.tanggal', 'orders.hari', 'orders.platform_id'])
            ->selectRaw('COALESCE(SUM(oi.price_after_discount * oi.quantity), 0) as total_order_value')
            ->groupBy('orders.id', 'orders.order_number', 'orders.tanggal', 'orders.hari', 'orders.platform_id')
            ->orderByDesc('orders.tanggal')
            ->limit(200)
            ->get();
        $mark('missing_orders_list');
            
        // Group transactions by order number for display
        $groupedTransactions = $transactions->groupBy('no_order');
        $mark('grouped_transactions');

        if ($profile) {
            Log::info('finance.shopee.index.profile.done', [
                'id' => $profileId,
                'marks_ms' => $profileMarks,
                'slow_queries' => $profileSlowQueries,
                'slow_queries_count' => count($profileSlowQueries),
                    ...
                    if (!$order) {
                        ...
                        if ($transactionExists) {
                            ...
                            if ($orderItems->isEmpty()) {
                                ...
                                // Group order items by tax_id for accurate preview
                                ...
                                foreach ($taxGroups as $taxId => $group) {
                                    ...
                                    $invoice = [
                                        'no_invoice' => $this->generateInvoiceForOrder($order, $taxId),
                                        'tax_id' => $taxId,
                                        'qty' => $group['total_qty'],
                                    ...
                                    $invoice['saldo_masuk'] = $total_saldo_masuk * $proportion;
    ...
    public function process(Request $request)
    {
        try {
            ...
            if (!empty($allOrderNumbers)) {
                ...
                // Get all orders with their items and fulfillment data in one query
                ...
                $existingTransRows = ShopeeFinancialTransaction::whereIn('no_order', $allOrderNumbers)
                    ->get(['no_order', 'no_invoice']);
                
                // Group by order and map to existing tax group (Category + Status)
                ...
                foreach ($existingTransRows as $row) {
                    ...
                    // Determine category from invoice prefix
                    $category = 'SKINCARE';
                    if (strpos($inv, 'HPNSDA-OLK') !== false) {
                        $category = 'KOPI';
                    }
                    ...
                    if ($status) {
                        $groupKey = $category . '_' . $status;
                        $existingTransactionsByOrder[$orderNo][$groupKey] = true;
                    } else {
            ...
            foreach ($processedRows as $rowInfo) {
                ...
                try {
                    ...
                    // Group OrderItems by tax_id from warehouse_stock (via barang_keluar)
                    // This handles multiple tax_ids (like HGN and LM) within the same order or bundling
                    $itemsByTaxId = [];
                    $totalQty = 0;
                    
                    foreach ($orderItems as $item) {
                        // Check if item has barang_keluar (fulfillment data)
                        $barangKeluars = $item->barangKeluar;
                        
                        if ($barangKeluars->count() > 0) {
                            $totalBkQty = (float) $barangKeluars->sum('qty');
                            
                            foreach ($barangKeluars as $bk) {
                                $taxId = ($bk->warehouseStock && $bk->warehouseStock->tax_id) ? $bk->warehouseStock->tax_id : 4; // Default to 4 (LM)
                                
                                if (!isset($itemsByTaxId[$taxId])) {
                                    $itemsByTaxId[$taxId] = [];
                                }
                                
                                // Calculate proportion of this item for this tax_id
                                $bkProportion = ($totalBkQty > 0) ? ($bk->qty / $totalBkQty) : 1;
                                
                                $itemsByTaxId[$taxId][] = [
                                    'item' => $item,
                                    'tax_id' => $taxId,
                                    'qty_portion' => $bk->qty,
                                    'proportion' => $bkProportion
                                ];
                            }
                        } else {
                            // Fallback if no fulfillment data yet
                            $taxId = ($item->warehouseStock && $item->warehouseStock->tax_id) ? $item->warehouseStock->tax_id : 4;
                            if (!isset($itemsByTaxId[$taxId])) {
                                $itemsByTaxId[$taxId] = [];
                            }
                            $itemsByTaxId[$taxId][] = [
                    ...
                    // Track all tax_id groups needed
            ...
            foreach ($validOrders as $orderId => $orderData) {
                ...
                try {
                    ...
                    // Calculate total invoice value for proportion calculation
                    $totalInvoiceValue = $order->orderItems->sum(function($item) {
                        return (float) $item->price_after_discount * (float) $item->quantity;
                    });
                    ...
                    foreach ($taxGroups as $taxId => $group) {
                        ...
                        $nominal_harga = round($group['total_nominal'], 2);
                        $nominal_diskon1 = round($total_nominal_diskon1 * $proportion, 2);
                        $nominal_diskon2 = round($total_nominal_diskon2 * $proportion, 2);
                        $nominal_diskon3 = round($total_nominal_diskon3 * $proportion, 2);
                        $nominal_diskon4 = round($total_nominal_diskon4 * $proportion, 2);
                        $nominal_diskon5 = round($total_nominal_diskon5 * $proportion, 2);
                        $nominal_diskon6 = round($total_nominal_diskon6 * $proportion, 2);
                        $nominal_diskon7 = round($total_nominal_diskon7 * $proportion, 2);
                        $nominal_diskon8 = round($total_nominal_diskon8 * $proportion, 2);
                        $nominal_diskon9 = round($total_nominal_diskon9 * $proportion, 2);
                        $nominal_diskon10 = round($total_nominal_diskon10 * $proportion, 2);
                        $nominal_diskon11 = round($total_nominal_diskon11 * $proportion, 2);
                        $nominal_diskon12 = round($total_nominal_diskon12 * $proportion, 2);
                        $saldo_masuk = round($total_saldo_masuk * $proportion, 2);
                        
                        // Calculate nominal_fix and outstanding for this tax group
                        $nominal_fix = $nominal_harga + $nominal_diskon1 + $nominal_diskon2 + $nominal_diskon3 + 
    ...
    protected function groupOrderItemsByTaxId($order)
    {
        ...
        // Pre-calculate total mapping quantities per order item for proportion calculation
        $totalMappingQtyByOrderItem = [];
        foreach ($barangKeluarItems as $bk) {
            $orderItemId = $bk->order_item_id;
            if (!isset($totalMappingQtyByOrderItem[$orderItemId])) {
                $totalMappingQtyByOrderItem[$orderItemId] = 0;
            }
            $totalMappingQtyByOrderItem[$orderItemId] += (float) $bk->qty;
        }
        ...
        foreach ($barangKeluarItems as $bk) {
            if ($bk->warehouseStock && $bk->warehouseStock->tax_id) {
                ...
                $taxGroups[$taxId]['total_qty'] += (float) $bk->qty;
                
                if ($bk->orderItem) {
                    $orderItemId = $bk->order_item_id;
                    $totalMappingQty = $totalMappingQtyByOrderItem[$orderItemId] ?? 0;
                    $proportion = ($totalMappingQty > 0) ? ((float) $bk->qty / $totalMappingQty) : 1;
                    
                    $itemTotalPrice = (float) $bk->orderItem->price_after_discount * (float) $bk->orderItem->quantity;
                    $taxGroups[$taxId]['total_nominal'] += $itemTotalPrice * $proportion;
                }
            }
        }
        ...
        if (empty($taxGroups)) {
            ...
            foreach ($orderItems as $item) {
                ...
                $taxGroups[$taxId]['total_qty'] += (float) $item->quantity;
                $taxGroups[$taxId]['total_nominal'] += (float) $item->price_after_discount * (float) $item->quantity;
            }
        }
    ...
    public function manual()
    {
        ...
        return view('financial.shopee.manual', compact('order'));
    }
    ...
    public function exportPdf(Request $request)
    {
        ...
        $query->whereNotExists(function($subQuery) {
            $subQuery->select(DB::raw(1))
                ...
                ->where(function ($q) {
                    $q->whereColumn('o.id', 'shopee_financial_transactions.order_id')
                      ->orWhereRaw('TRIM(o.order_number) = TRIM(shopee_financial_transactions.no_order)');
                })
                ->groupBy('o.id')
                ->havingRaw('(
                    (SELECT COALESCE(SUM(quantity), 0) FROM order_items WHERE order_id = o.id) <= 0.0001
                    OR
                    (SELECT COALESCE(SUM(bk.qty), 0)
                        FROM barang_keluar bk
                        JOIN order_items oi ON oi.id = bk.order_item_id
                        WHERE oi.order_id = o.id
                    ) <= (
                        (SELECT COALESCE(SUM(rpd.qty), 0)
                            FROM retur_penjualan_details rpd
                            JOIN retur_penjualans rp2 ON rp2.id = rpd.retur_penjualan_id
                            WHERE rp2.order_id = o.id AND rp2.status IN (\'draft\', \'selesai\')
                        ) + 0.0001
                    )
                )');
        });
