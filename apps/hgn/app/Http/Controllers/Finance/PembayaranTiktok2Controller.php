...
class PembayaranTiktok2Controller extends Controller
{
    ...
    {
        // Get platform by ID (tiktok2 = ID 7)
        ...
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) LIKE ?', ['%tiktok2%'])->first();
        }
        ...
        $missingOrdersBase = Order::withoutGlobalScope('mainCategory')
            ->leftJoin('tiktok2_financial_transactions as sft', 'orders.id', '=', 'sft.order_id')
        ...
        $missingOrdersNominal = cache()->remember(
            'tiktok2_missing_nominal_' . (string) ($mainCategoryId ?? 'all'),
            300,
    ...
    public function process(Request $request)
    {
        try {
            ...
            if (!empty($allOrderNumbers)) {
                ...
                $existingTransRows = Tiktok2FinancialTransaction::whereIn('no_order', $allOrderNumbers)
            ...
            // Sekarang, dapatkan nomor invoice untuk setiap kelompok tax_id secara batch
            ...
            foreach ($validTaxGroups as $taxId => $count) {
                ...
                // Tentukan kategori dan status pajak berdasarkan tax_id
                $category = in_array($taxId, [1, 2, 5, 6]) ? InvoiceSequence::CATEGORY_KOPI : InvoiceSequence::CATEGORY_SKINCARE;
                $taxStatus = in_array($taxId, [1, 3, 5, 7]) ? InvoiceSequence::TAX_PKP : InvoiceSequence::TAX_NON_PKP;
            ...
            foreach ($validOrders as $orderId => $orderData) {
                ...
                try {
                    ...
                    $totalOrderPrice = 0;
                    foreach ($order->orderItems as $item) {
                        $totalOrderPrice += (float) $item->price_after_discount * (float) $item->quantity;
                    }
                    ...
                    foreach ($itemsByTaxId as $taxId => $itemsData) {
                        // Determine category and status for this tax_id to check existing transactions
                        $category = in_array($taxId, [1, 2, 5, 6]) ? 'KOPI' : 'SKINCARE';
                        $taxStatus = in_array($taxId, [1, 3, 5, 7]) ? 'PKP' : 'NON_PKP';
                        $groupKey = $category . '_' . $taxStatus;
                        
                        // Skip if this specific category + status already has a transaction for this order
                        $existingForOrder = $existingTransactionsByOrder[$order->order_number] ?? [];
                        if (!empty($existingForOrder[$groupKey])) {
                            continue;
                        }
                        
                        // Get the next invoice number from the pre-generated batch for this tax_id
                        $invoiceData = array_shift($invoiceNumbersByTaxId[$taxId]);
                        if (!$invoiceData) {
                            throw new \Exception("No invoice number available for tax_id $taxId");
                        }
                        ...
                        foreach ($itemsData as $itemData) {
                            $item = $itemData['item'];
                            $qtyPortion = (float) $itemData['qty_portion'];
                            $itemProportion = (float) $itemData['proportion']; // Proportion from mapping
                            
                            $groupQty += $qtyPortion;
                            
                            // Group value for this item = Total Item Price * Proportion of this mapping
                            $itemTotalPrice = (float) $item->price_after_discount * (float) $item->quantity;
                            $groupValue += $itemTotalPrice * $itemProportion;
                        }
                        
                        // Calculate final proportion of this group compared to TOTAL order price
                        $finalProportion = ($totalOrderPrice > 0) ? $groupValue / $totalOrderPrice : 1;
                        
                        // Set values based on group value and final proportion
                        $transaction->nominal_harga = round($groupValue, 2);
                        $transaction->qty = $groupQty;
                        $transaction->nominal_diskon1 = round($nominal_diskon1 * $finalProportion, 2);
                        $transaction->nominal_diskon2 = round($nominal_diskon2 * $finalProportion, 2);
                        $transaction->nominal_diskon3 = round($nominal_diskon3 * $finalProportion, 2);
                        $transaction->nominal_diskon4 = round($nominal_diskon4 * $finalProportion, 2);
                        $transaction->nominal_diskon5 = round($nominal_diskon5 * $finalProportion, 2);
                        $transaction->nominal_diskon6 = round($nominal_diskon6 * $finalProportion, 2);
                        $transaction->nominal_diskon7 = round($nominal_diskon7 * $finalProportion, 2);
                        $transaction->nominal_diskon8 = round($nominal_diskon8 * $finalProportion, 2);
                        $transaction->nominal_diskon9 = round($nominal_diskon9 * $finalProportion, 2);
                        $transaction->nominal_diskon10 = round($nominal_diskon10 * $finalProportion, 2);
                        $transaction->nominal_diskon11 = round($nominal_diskon11 * $finalProportion, 2);
                        $transaction->nominal_diskon12 = round($nominal_diskon12 * $finalProportion, 2);
                        ...
                        // Set saldo_masuk applying proportion
                        $totalSaldoMasuk = isset($rowData['JUMLAH MASUK PEMBAYARAN']) ? (float) $rowData['JUMLAH MASUK PEMBAYARAN'] : 0;
                        $transaction->saldo_masuk = round($totalSaldoMasuk * $finalProportion, 2);
    ...
    public function manual()
    {
        ...
        return view('financial.tiktok2.manual', compact('order'));
    }
