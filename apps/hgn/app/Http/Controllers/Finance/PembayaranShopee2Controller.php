...
class PembayaranShopee2Controller extends Controller
{
    ...
    {
        // Get platform by ID (shopee2 = ID 6)
        $platformModel = Platform::find(6);
        
        // Jika tidak ditemukan berdasarkan ID, cari berdasarkan nama dengan case-insensitive
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) = ?', ['shopee2'])->first();
        }
        
        // Jika masih tidak ditemukan, cari dengan LIKE
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) LIKE ?', ['%shopee2%'])->first();
        }
        
        if (!$platformModel) {
            return redirect()->back()->with('error', 'Platform shopee2 tidak ditemukan');
        }
        $platformId = $platformModel->id;
        $platform = 'shopee2'; // Set platform name for view
        ...
        $missingOrdersBase = Order::withoutGlobalScope('mainCategory')
            ->leftJoin('shopee2_financial_transactions as sft', 'orders.id', '=', 'sft.order_id')
        ...
        $missingOrdersNominal = cache()->remember(
            'shopee2_missing_nominal_' . (string) ($mainCategoryId ?? 'all'),
            300,
    ...
    public function process(Request $request)
    {
        try {
            ...
            if (!empty($allOrderNumbers)) {
                ...
                $existingTransRows = Shopee2FinancialTransaction::whereIn('no_order', $allOrderNumbers)
            ...
            foreach ($validOrders as $orderId => $orderData) {
                ...
                try {
                    ...
                    foreach ($taxGroups as $taxId => $group) {
                        ...
                        $proportion = $totalOrderPrice > 0 ? ($group['total_nominal'] / $totalOrderPrice) : (1 / count($taxGroups));
    ...
    public function manual()
    {
        ...
        return view('financial.shopee2.manual', compact('order'));
    }
