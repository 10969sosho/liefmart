...
class UnpaidOrdersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $buildQuery = function($isBlibli = false) use ($blibliPlatformId, $filters) {
            ...
            $query->havingRaw('(ROUND(returned_quantity, 2) < ROUND(order_total_quantity, 2) OR returned_quantity = 0)');
        ...
        // Calculate total pages to determine if we're on the last page
        $totalPages = ceil($totalCount / $perPage);
        
        // If we're on the last page or near it, load more data to ensure we have enough
        // For last page, we need to load at least enough to cover all remaining items
        if ($page >= $totalPages - 1) {
            // On last page or second to last, load enough to cover all remaining items
            $loadLimit = max($perPage * 3, ($page * $perPage) + ($perPage * 2), $totalCount);
        } else {
            // For earlier pages, use a reasonable limit
        ...
        // If we don't have enough results and we're not on the last page, try loading more
        if ($paginatedResults->isEmpty() && $page < $totalPages) {
            // Load more data - increase limit significantly
            $loadLimit = min($totalCount, ($page * $perPage) + ($perPage * 5));
            $resultsNonBlibli = $queryNonBlibli ? $queryNonBlibli->limit($loadLimit)->get() : collect();
            $resultsBlibli = $queryBlibli ? $queryBlibli->limit($loadLimit)->get() : collect();
        ...
        $paginatedOrders = $paginatedResults->map(function($row) use ($orders) {
            ...
            if ($order) {
                ...
                $order->is_return_unpaid = (bool)($row->is_return_unpaid ?? 0);
                $order->unpaid_reason = $row->unpaid_reason ?? null;
            }
            return $order;
        })->filter();
    ...
    public function exportPdf(Request $request)
    {
        ...
        if (!$platform) {
            ...
            $queryNonBlibli = Order::withoutGlobalScope('mainCategory')
            ...
            $queryNonBlibli = $isBlibli ? null : Order::withoutGlobalScope('mainCategory')
                ->with(['orderItems.warehouseStock.tax', 'platform', 'mainCategory'])
                ->whereDoesntHave('shopeeFinancialTransactions')
                ->whereDoesntHave('tokopediaFinancialTransactions')
                ->whereDoesntHave('tiktokFinancialTransactions')
        ...
        $allOrders = $allOrders->filter(function($order) {
            ...
            if ($hasReturSelesai) {
                $hasFinancialTransaction = $order->shopeeFinancialTransactions()->exists() ||
                    $order->tokopediaFinancialTransactions()->exists() ||
                    $order->tiktokFinancialTransactions()->exists() ||
                    $order->blibliFinancialTransactions()->exists();
    ...
    private function calculateSummaryFromSQL($query)
    {
        ...
        // Hanya tampilkan retur sebagian, retur full tidak akan muncul
        $baseSql .= "
            ...
            HAVING (ROUND(returned_quantity, 2) < ROUND(order_total_quantity, 2) OR returned_quantity = 0)
        ";
