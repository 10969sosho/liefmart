...
class PembayaranBlibliController extends Controller
{
    public function index(Request $request)
    {
        ...
        $missingOrdersNominal = 0;
        $missingOrders = collect();

        if ($blibliPlatformId) {
            $missingOrdersBase = Order::withoutGlobalScope('mainCategory')
                ->leftJoin('blibli_financial_transactions as sft', 'orders.id', '=', 'sft.order_id')
                ...
                ->where('orders.platform_id', $blibliPlatformId)
                ->where('oi.quantity', '>', 0);
            ...
            $missingOrdersNominal = cache()->remember(
                'blibli_missing_nominal_' . (string) ($mainCategoryId ?? 'all'),
                300,
    ...
    public function previewDuplicateRemoved(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $issues = session('blibli_import_issues');
            ...
            return view('financial.blibli.preview', compact('data', 'previewData', 'previewHeaders', 'headerLabels', 'issues', 'totalRows', 'validRows', 'invalidRows'));
        }
        ...
        try {
            ...
            Log::info('Starting Blibli financial import preview', [
            ...
            // Standardize header names for Blibli
            $headerMapping = [
                'Order ID' => 'ORDER_ID',
            ...
            if (!empty($orderNumbers)) {
                ...
                $existingTransactions = BlibliFinancialTransaction::whereIn('no_order', $orderNumbers)
                    ->pluck('no_order')
                    ->toArray();
            ...
            session([
                'blibli_import_data' => $data,
                'blibli_preview_data' => $previewData,
                'blibli_preview_headers' => $standardizedHeaders,
                'blibli_header_labels' => $headers,
                'blibli_import_issues' => $issues,
                'blibli_total_rows' => $totalRows,
                'blibli_valid_rows' => count(array_filter($data, function($row) { return $row['_valid']; })),
                'blibli_invalid_rows' => count(array_filter($data, function($row) { return !$row['_valid']; }))
            ]);
            
            // Generate and store process token for secure processing
            $processToken = uniqid('blibli_', true);
            session(['blibli_process_token' => $processToken]);
            
            return view('financial.blibli.preview', compact('data', 'previewData', 'previewHeaders', 'headerLabels', 'issues', 'totalRows', 'validRows', 'invalidRows'));
            
        } catch (\Exception $e) {
            Log::error("Error during Blibli financial import preview: " . $e->getMessage());
    ...
    {
        // Get Blibli platform ID (ID 4 for Blibli)
        $blibliPlatformId = 4;
        ...
        if ($orderId) {
            $order = Order::where('platform_id', $blibliPlatformId)->find($orderId);
        }
        
        return view('financial.blibli.manual', compact('order'));
    }
    ...
    public function storeManual(Request $request)
    {
        ...
        try {
            ...
            // Check if order is from Blibli platform (by platform_id)
            if (!$order->platform_id || $order->platform_id !== $blibliPlatformId) {
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'Nomor pesanan yang dipilih bukan dari platform Blibli.');
            }
