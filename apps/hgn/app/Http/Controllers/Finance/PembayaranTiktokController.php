...
class PembayaranTiktokController extends Controller
{
    ...
    {
        // Get platform by ID (tiktok = ID 3)
        $platformModel = Platform::find(3);
        
        // Jika tidak ditemukan berdasarkan ID, cari berdasarkan nama dengan case-insensitive
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) = ?', ['tiktok'])->first();
        }
        
        // Jika masih tidak ditemukan, cari dengan LIKE
        if (!$platformModel) {
            $platformModel = Platform::whereRaw('LOWER(name) LIKE ?', ['%tiktok%'])
                ->whereRaw('LOWER(name) NOT LIKE ?', ['%tiktok2%'])
                ->first();
        }
        
        if (!$platformModel) {
            return redirect()->back()->with('error', 'Platform tiktok tidak ditemukan');
        }
        $platformId = $platformModel->id;
        $platform = 'tiktok'; // Set platform name for view
        ...
        $query->whereHas('order', function ($orderQuery) {
            $orderQuery->whereRaw('(SELECT COALESCE(SUM(quantity), 0) FROM order_items WHERE order_id = orders.id) > 0');
        });
    ...
    public function manual()
    {
        ...
        return view('financial.tiktok.manual', compact('order'));
    }
