...
class PembayaranTokopediaController extends Controller
{
    public function index(Request $request)
    {
        ...
        $platformModel = Platform::whereRaw('LOWER(name) = ?', ['tokopedia'])->first();
        $platformId = $platformModel ? $platformModel->id : null;
        ...
        if ($platformId) {
            $missingOrdersBase = Order::withoutGlobalScope('mainCategory')
                ->leftJoin('tokopedia_financial_transactions as sft', 'orders.id', '=', 'sft.order_id')
            ...
            $missingOrdersNominal = cache()->remember(
                'tokopedia_missing_nominal_' . (string) ($mainCategoryId ?? 'all'),
                300,
    ...
    {
        // Get Tokopedia platform ID (ID 2 for Tokopedia)
        $tokopediaPlatformId = 2;
        ...
        if ($orderId) {
            $order = Order::where('platform_id', $tokopediaPlatformId)->find($orderId);
        }
        
        return view('financial.tokopedia.manual', compact('order'));
    }
    ...
    public function storeManual(Request $request)
    {
        ...
        try {
            ...
            // Check if order is from Tokopedia platform (by platform_id)
            if (!$order->platform_id || $order->platform_id !== $tokopediaPlatformId) {
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'Nomor pesanan yang dipilih bukan dari platform Tokopedia.');
            }
