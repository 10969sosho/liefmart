...
class SalesAnalyticsController extends Controller
{
    public function salesByPlatformReport(Request $request)
    {
        ...
        $totalReturns = ReturPenjualan::whereIn('status', ['draft', 'selesai'])->whereIn('order_id', function($q) use ($startDateCarbon, $endDateCarbon, $request) {
            $q->select('id')
              ->from('orders')
              ->whereNotNull('platform_id')
              ->whereBetween('tanggal', [$startDateCarbon, $endDateCarbon]);
    ...
    public function salesExportMapped(Request $request)
    {
        ...
        if (!empty($orderIds)) {
            ...
            $orderItemIds = $orders->pluck('orderItems')->flatten()->pluck('id')->filter()->values();
            $returQtyByOrderItemId = collect();
            if ($orderItemIds->isNotEmpty()) {
                $returQtyByOrderItemId = \App\Models\ReturPenjualanDetail::query()
            ...
            $orders->each(function($order) use ($returQtyByOrderItemId) {
                foreach ($order->orderItems as $item) {
                    $qtyReturIndividual = (float) ($returQtyByOrderItemId[$item->id] ?? 0);

                    $effectiveMappings = collect();
                    $packageQuantity = 1.0;
                    if ($item->platformProduct && $item->platformProduct->mappingBarang) {
                        $mappings = $item->platformProduct->mappingBarang;
                        $orderCreatedAt = $order->created_at ?? $item->created_at;

                        $validMappings = $mappings->filter(function($mapping) use ($orderCreatedAt) {
                            $effectiveAt = $mapping->valid_from ?? $mapping->created_at;
                            return $effectiveAt && $effectiveAt->lte($orderCreatedAt);
                        });

                        $effectiveAtTimestamp = $validMappings->max(function($mapping) {
                            $effectiveAt = $mapping->valid_from ?? $mapping->created_at;
                            return $effectiveAt ? $effectiveAt->getTimestamp() : null;
                        });

                        if ($effectiveAtTimestamp !== null) {
                            $effectiveMappings = $mappings->filter(function($mapping) use ($effectiveAtTimestamp) {
                                $effectiveAt = $mapping->valid_from ?? $mapping->created_at;
                                return $effectiveAt && $effectiveAt->getTimestamp() === $effectiveAtTimestamp;
                            })->values();
                        } else {
                            $effectiveMappings = $mappings->where('is_active', true)->values();
                        }

                        $packageQuantity = $effectiveMappings->count() > 0 ? (float) $effectiveMappings->sum('quantity') : 1.0;
                    }
                    ...
                    $item->effective_mappings = $effectiveMappings;
                    $item->qty_retur = $qtyRetur;
                    $item->original_qty = $originalQty;
                    $item->original_total_item = round(((float) ($item->price_after_discount ?? 0)) * $originalQty, 2);
                }
            });
        }
    ...
    public function exportSalesByPlatform(Request $request)
    {
        ...
        $orders = $orders->filter(function($order) {
            return (float) ($order->total_volume ?? 0) > 0;
        })->values();

        // Get order IDs for returns query
        $orderIds = $orders->pluck('id')->toArray();
        
        $totalReturns = ReturPenjualan::whereIn('status', ['draft', 'selesai'])
        ...
        return Excel::download(new SalesByPlatformExport($orders->values(), $summary, $startDate, $endDate, $request->platform_id), $filename);
    }
