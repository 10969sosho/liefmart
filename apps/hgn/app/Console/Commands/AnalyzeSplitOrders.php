...
class AnalyzeSplitOrders extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("Logic: If Order has MIXED Tax IDs (e.g. HGN & LM), Split is CORRECT.");
        $this->info("       If Order has SINGLE Tax ID (e.g. Only HGN), Split is INCORRECT.\n");
        ...
        foreach ($problemOrders as $noOrder) {
            ...
            // Mapping check (3=HGN, 4=LM)
            $hasHGN = in_array(3, $uniqueTaxIds);
            $hasLM = in_array(4, $uniqueTaxIds);
            
            if ($isMixed) {
                $correctlySplit[] = [
                    'order' => $noOrder,
                    'tax_ids' => $taxIdString,
                    'note' => ($hasHGN && $hasLM) ? 'Contains HGN & LM (Correct)' : 'Mixed Tax IDs'
                ];
            } else {
                $trueErrors[] = [
                    'order' => $noOrder,
        ...
        $this->info("\n=== CORRECTLY SPLIT ORDERS (Mixed HGN & LM) ===");
