...
class ResyncAllInvoices extends Command
{
    ...
    public function handle()
    {
        ...
        $platforms = [
            ...
            'Tokopedia' => \App\Models\TokopediaFinancialTransaction::class,
            'Blibli' => \App\Models\BlibliFinancialTransaction::class,
            'Lazada' => \App\Models\LazadaFinancialTransaction::class,
        ...
        $processedInvoices = $invoices->map(function($inv) {
            ...
            if (count($parts) >= 4) {
                ...
                if (str_contains($suffix, 'HGNSDA')) {
                    $category = InvoiceSequence::CATEGORY_SKINCARE;
                }
                
                // Map Sales Type
                if (str_contains($suffix, 'OL')) {
                    $salesType = InvoiceSequence::SALES_ONLINE;
                }
                if ($suffix == 'HPNSDA-KOP' || $suffix == 'HGNSDA-KOS') {
        ...
        foreach ($processedInvoices as $inv) {
            ...
            // Generate Suffix
            $suffix = '';
            if ($inv['category'] === InvoiceSequence::CATEGORY_KOPI) {
                if ($inv['sales_type'] === InvoiceSequence::SALES_ONLINE) {
                    $suffix = 'HPNSDA-OLK';
                } else {
                    $suffix = 'HPNSDA-KOP';
                }
            ...
            // Format Number
            $counterLength = ($inv['sales_type'] === InvoiceSequence::SALES_OFFLINE) ? 4 : 6;
