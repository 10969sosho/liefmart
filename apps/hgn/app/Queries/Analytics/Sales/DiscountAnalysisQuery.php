                ...
                SELECT 
                    ...
                    (
                        COALESCE((
                            SELECT SUM(
                                COALESCE((
                                    SELECT pipv2.initial_price
                                    FROM product_initial_price_versions pipv2
                                    WHERE pipv2.product_id = p2.id
                            ...
                            WHERE bk2.order_item_id = oi.id
                        ), 0) / NULLIF(oi.quantity, 0)
                    ) as original_price,
                    ...
                    (
                        COALESCE((
                            ...
                            INNER JOIN products p2 ON p2.id = ws2.product_id
                            WHERE bk2.order_item_id = oi.id
                        ), 0) - (oi.price_after_discount * oi.quantity)
                    ) as item_discount,
                    CASE 
                        WHEN COALESCE((
                            SELECT SUM(
    ...
    public static function buildCount(array $filters = []): string
    {
        ...
        if ($minDiscount !== null) {
            $discountFilter .= " AND (COALESCE((
                ...
                INNER JOIN products p2 ON p2.id = ws2.product_id
                WHERE bk2.order_item_id = oi.id
            ), 0) - (oi.price_after_discount * oi.quantity)) >= " . floatval($minDiscount);
        }
        if ($maxDiscount !== null) {
            $discountFilter .= " AND (COALESCE((
                SELECT SUM(
                ...
                INNER JOIN products p2 ON p2.id = ws2.product_id
                WHERE bk2.order_item_id = oi.id
            ), 0) - (oi.price_after_discount * oi.quantity)) <= " . floatval($maxDiscount);
        }
