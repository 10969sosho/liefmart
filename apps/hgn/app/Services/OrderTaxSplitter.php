...
class OrderTaxSplitter
{
    ...
    public function splitOrder(Order $order)
    {
        ...
        foreach ($order->orderItems as $item) {
            $this->processOrderItem($item, $taxGroups);
        }
    ...
    private function processOrderItem(OrderItem $item, &$taxGroups)
    {
        ...
        foreach ($bks as $bk) {
            ...
            $effectiveAt = $item->order?->tanggal ?? $item->order?->created_at ?? $item->created_at ?? now();
            $cacheKey = $product->id . '|' . (string) $effectiveAt;

            if (!array_key_exists($cacheKey, $this->priceCache)) {
                $this->priceCache[$cacheKey] = ProductInitialPriceVersion::getInitialPriceForDate($product->id, $effectiveAt);
            }

            $price = $this->priceCache[$cacheKey] > 0 ? $this->priceCache[$cacheKey] : 0;
            $value = $bk->qty * $price;
