...
@section('page-title', 'Arus Kas Blibli')

@section('content')
<div class="ds-page-header">
    <div>
        <h1 class="text-gradient">Data Arus Kas Blibli</h1>
    </div>
    <div>
        <a href="{{ route('finance.aruskasblibli.import') }}" class="btn btn-primary">
...
<div class="card shadow-sm">
    <div class="card-body">
                    ...
                    <!-- Filter Form -->
                    <form action="{{ route('finance.aruskasblibli.index') }}" method="GET" class="mb-4">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-2 d-flex align-items-end">
                                <div class="form-group w-100">
                                    <a href="{{ route('finance.aruskasblibli.index') }}" class="btn btn-secondary w-100">
                            ...
                            <tbody>
                                @forelse ($transactions as $index => $transaction)
                                    <tr>
                                        ...
                                        <td class="tanggal-col fw-bold">{{ $transaction->tanggal_pembayaran ? \Carbon\Carbon::parse($transaction->tanggal_pembayaran)->format('d-m-Y') : '-' }}</td>
                                        <td class="deskripsi-col">{{ $transaction->deskripsi }}</td>
                                        <td class="nopesanan-col fw-bold">{{ $transaction->no_pesanan }}</td>
                                        <td class="tglpesanan-col">{{ $transaction->tanggal_pesanan ? \Carbon\Carbon::parse($transaction->tanggal_pesanan)->format('d-m-Y') : '-' }}</td>
                                        <td class="pembayaran-col text-end fw-bold">Rp {{ number_format($transaction->pembayaran, 0, ',', '.') }}</td>
                                    ...
                                    <tr>
                                        <td colspan="7" class="text-center py-4">
                                            <div class="d-flex flex-column align-items-center">
                                                ...
                                                <a href="{{ route('finance.aruskasblibli.import') }}" class="btn btn-sm btn-primary mt-2">
                                                    <i class="fas fa-file-import me-1"></i> Import Data
                                                </a>
                                            </div>
                                        </td>
