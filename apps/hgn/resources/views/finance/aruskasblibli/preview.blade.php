...
@section('page-title', 'Preview Import Arus Kas Blibli')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Preview Data Arus Kas Blibli</h4>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle me-2"></i> Kolom <strong>Tanggal Pembayaran</strong>, <strong>Deskripsi</strong>, <strong>Pembayaran</strong>, dan <strong>Saldo Akhir</strong> adalah kolom utama yang akan ditampilkan.
                    </div>
                            ...
                            <tbody>
                                @foreach ($data as $index => $row)
                                    <tr class="{{ $row['_valid'] ? '' : 'table-danger' }}">
                                        ...
                                        <td class="fw-bold">{{ $row['Tanggal Pembayaran'] }}</td>
                                        <td>{{ $row['Deskripsi'] }}</td>
                                        <td class="fw-bold">{{ $row['No. Pesanan'] }}</td>
                                        <td>{{ $row['Tanggal pesanan'] ?? '-' }}</td>
                                        <td class="fw-bold text-end">{{ number_format((float) $row['Pembayaran'], 2, ',', '.') }}</td>
                                        <td class="fw-bold text-end">{{ number_format((float) $row['Saldo Akhir'], 2, ',', '.') }}</td>
                                        <td>
                                            @if (!$row['_valid'])
                    ...
                    <div class="mt-4 card bg-light shadow-sm border-0">
                        <div class="card-body">
                            <form id="processForm" action="{{ route('finance.aruskasblibli.process') }}" method="POST">
                                @csrf
                                <input type="hidden" name="import_session_id" value="{{ $import_session_id ?? '' }}" id="importSessionId">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <a href="{{ route('finance.aruskasblibli.import') }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form && saveButton) {
        form.addEventListener('submit', function(e) {
            console.log('=== BLIBLI FORM SUBMIT START ===');
