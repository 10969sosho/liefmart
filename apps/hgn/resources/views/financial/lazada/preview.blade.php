...
@section('title', 'Preview Data Keuangan Lazada')
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">Preview Data Keuangan Lazada</h3>
                    <div class="card-tools">
                        <a href="{{ route('finance.lazada.import') }}" class="btn btn-default btn-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            @php
                                $bankInfo = method_exists(\App\Models\LazadaFinancialTransaction::class, 'getBankAccountInfo') 
                                    ? \App\Models\LazadaFinancialTransaction::getBankAccountInfo() 
                                    : ['bank_name' => '-', 'account_number' => '-', 'account_name' => '-', 'has_active' => false];
                            @endphp
                            <div class="row">
                                <div class="col-md-4">
                    ...
                    <div class="alert alert-info">
                        ...
                        <ul>
                            <li>Total data: {{ $totalRows ?? ($stats['total_rows'] ?? 0) }}</li>
                            <li>Data valid: {{ $validRows ?? ($stats['processed_rows'] ?? 0) }}</li>
                            <li>Data tidak valid: {{ $invalidRows ?? ($stats['invalid_data'] ?? 0) }}</li>
                        </ul>
                    </div>

                    @if(($invalidRows ?? ($stats['invalid_data'] ?? 0)) > 0)
                    <div class="alert alert-warning">
                        <h5><i class="icon fas fa-exclamation-triangle"></i> Masalah Ditemukan ({{ $invalidRows ?? ($stats['invalid_data'] ?? 0) }} masalah)</h5>
                            ...
                            <ul>
                                ...
                                @elseif(!empty($invalidData))
                                    @foreach($invalidData as $invalid)
                                        <li>{{ is_array($invalid) ? implode(', ', $invalid) : $invalid }}</li>
                                    @endforeach
                                @else
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-sm">
                            ...
                            <tbody>
                                ...
                                @php
                                    $isValid = true;
                                    if (isset($data) && is_array($data) && isset($data[$index]['_valid'])) {
                                        $isValid = $data[$index]['_valid'];
                                    } elseif (isset($row['_valid'])) {
                                        $isValid = $row['_valid'];
                                    }
                                @endphp
                                @if($isValid)
                                <tr class="{{ $index % 2 == 0 ? 'table-primary' : '' }}">
                                    <td>
                                        @if(isset($row['tanggal_order']) && $row['tanggal_order'] !== 'N/A' && $row['tanggal_order'])
                                            @php
                                    ...
                                    <td class="text-end">{{ isset($row['nominal_diskon5']) ? number_format($row['nominal_diskon5'] ?? 0, 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['nominal_diskon6']) ? number_format($row['nominal_diskon6'] ?? 0, 0, ',', '.') : '-' }}</td>
                                    <td class="text-end">{{ isset($row['adjustment']) ? number_format($row['adjustment'] ?? 0, 0, ',', '.') : '-' }}</td>
                                    ...
                                    <td>
                                        @if(isset($row['tanggal_masuk_pembayaran']) && $row['tanggal_masuk_pembayaran'] !== 'N/A' && $row['tanggal_masuk_pembayaran'])
                                            @php
                    ...
                    <div class="mt-3">
                        <form action="{{ route('finance.lazada.process') }}" method="POST" class="d-inline">
                            ...
                            <input type="hidden" name="process_token" value="{{ session('lazada_process_token', uniqid()) }}">
                            
                            @php
                                $validRowsCount = $validRows ?? ($stats['processed_rows'] ?? 0);
                            @endphp
                            <button type="submit" class="btn btn-success" {{ $validRowsCount == 0 ? 'disabled' : '' }}>
                                <i class="fas fa-check"></i> Proses {{ $validRowsCount }} Data Valid
                            </button>
                            
                            <a href="{{ route('finance.lazada.import') }}" class="btn btn-secondary">
                                <i class="fas fa-arrow-left"></i> Kembali
                            </a>
                        </form>
                    </div>
