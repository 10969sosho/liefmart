                                ...
                                <tbody>
                                    ...
                                    @forelse($orders as $order)
                                        @php
                                            ...
                                            $grossTotal = $orderItems->sum(function($item) {
                                                return $item->price_after_discount * $item->quantity;
                                            });
                                            
                                            // Calculate returned total
                                            $returnedTotal = $orderItems->sum(function($item) {
                                                // Get returned quantity from eager loaded relation
                                                $returnedQtyIndividual = 0;
                                                if ($item->relationLoaded('returPenjualanDetails')) {
                                                    $returnedQtyIndividual = $item->returPenjualanDetails->filter(function($detail) {
                                                ...
                                                $packageQuantity = 1;
                                                if ($item->platformProduct && $item->platformProduct->relationLoaded('mappingBarang') && $item->platformProduct->mappingBarang->count() > 0) {
                                                    $packageQuantity = $item->platformProduct->mappingBarang
                                                        ->where('is_active', true)
                                                        ->sum('quantity');
                                                }
                                                
                                                $returnedQty = $packageQuantity > 0 ? $returnedQtyIndividual / $packageQuantity : $returnedQtyIndividual;
                                                
                                                return $returnedQty * $item->price_after_discount;
                                            });
                                            
                                            $orderTotal = $grossTotal - $returnedTotal;
