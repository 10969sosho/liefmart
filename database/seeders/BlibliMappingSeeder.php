...
class BlibliMappingSeeder extends Seeder
{
    public function run()
    {
        $this->command->info('Starting BlibliMappingSeeder...');

        // Path to the mapping file
        $filePath = storage_path('app/imports/mapping/BLIBLI.xlsx - mapping BLIBLI.csv');
        ...
        // Get platform ID for Blibli
        $blibliPlatformId = Platform::where('name', 'blibli')->value('id');
        
        if (!$blibliPlatformId) {
            $this->command->error('BLIBLI platform not found in the database.');
            return;
        }
        
        // Clear existing mappings first
        $platformProductIds = PlatformProduct::where('platform_id', $blibliPlatformId)->pluck('id')->toArray();
        ...
        if (!empty($platformProductIds)) {
            ...
            $this->command->info("Deleted {$deletedCount} existing BLIBLI mappings");
        } else {
            $this->command->info("No existing BLIBLI platform products found");
        }
        ...
        $this->command->info("BlibliMappingSeeder completed. Total: $total, Success: $success, Failed: $failed");
