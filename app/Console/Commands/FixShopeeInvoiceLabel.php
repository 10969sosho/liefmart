...
class FixShopeeInvoiceLabel extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($transactions as $trx) {
            ...
            // Logic to identify PKP (Tax ID 3) transaction
            // We know from analysis that 103500 is the PKP item
            if ($trx->nominal_harga == 103500) {
                $this->info("  -> Identified as PKP Item (Tax ID 3).");
                ...
                if (str_ends_with($trx->no_invoice, '/02')) {
                 ...
                 $this->info("  -> Assumed NON PKP/Other (Tax ID 4). Status /02 is likely correct.");
            }
        }
    ...
    private function regenerateInvoice($trx)
    {
        DB::beginTransaction();
        try {
            // Params for PKP (Tax ID 3) -> Skincare, Online, PKP
