 ...
 * SalesVolumeQuery
 * 
 * Query untuk analytics Sales Volume Report
 * Similar to SalesByPlatform but focused on volume analysis
 */
class SalesVolumeQuery
{
    /**
     * Build query untuk sales volume report
     */
    public static function build(array $filters = []): string
    {
        ...
        return "
            ...
            ORDER BY ot.order_total_volume DESC, o.tanggal DESC";
    }
        ...
        return "
            ...
            SELECT 
                ...
                CASE 
                    ...
                    THEN COALESCE(SUM(ot.order_total_volume), 0) / COUNT(DISTINCT o.id)
                    ELSE 0
